...
class SlideShowPics(QtGui.QMainWindow):
    ...
    def showImageByPath(self, path):
        if path:
            ...
            opef = QtGui.QGraphicsOpacityEffect()
            anim = QtCore.QPropertyAnimation(opef, "opacity")
            self.label.setGraphicsEffect(opef)

            image = QtGui.QImage(path)
            pp = QtGui.QPixmap.fromImage(image)
            opef.setOpacity(1)
            '''anim.setDuration(2000)
            anim.setStartValue(self.opef.opacity)
            anim.setEndValue(1)
            anim.start()'''
    ...
    def keyPressEvent(self, keyevent):
        ...
        if event == QtCore.Qt.Key_Right:
            ...
            opef.setOpacity(.5)
            print opef.opacity()
            self.label.setGraphicsEffect(self.opef)
            self.nextImage()
        if event == 32:
...
if __name__ == '__main__':
    curntPaths = "/home/john/Documents/raspi-slideshow/pyqt/slides"
    if len(sys.argv) > 1:
